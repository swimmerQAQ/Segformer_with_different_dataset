...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim,  num_heads, window_size=7, shift_size=0,
        ...
        #             img_mask[:, h, w, :] = cnt
        #             cnt += 1
        #     # print("one patch img size:" , img_mask.shape)
        #     # quit()
    ...
    def forward(self, x):
        # print("SwinTransformerBlock:",H, W , x.shape)
        B, L, C = x.shape
        H, W = self.H, self.W
        # quit()
        ...
        if self.shift_size > 0:
            ...
            for h in h_slices:
                for w in w_slices:
                    img_mask[:, h, w, :] = cnt
                    cnt += 1
            # print("one patch img size:" , img_mask.shape, self.window_size)
            # quit()
            mask_windows = window_partition(img_mask, self.window_size)  # nW, window_size, window_size, 1
            mask_windows = mask_windows.view(-1, self.window_size * self.window_size)
            attn_mask = mask_windows.unsqueeze(1) - mask_windows.unsqueeze(2)
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        # after pading torch.Size([2, 192, 256, 256])   
        # test after pading torch.Size([1, 192, 128, 256]) 
        # print(x.size())
...
class SwinTransformerV2(nn.Module):
    ...
    def forward_features(self, x):
        ...
        # x = torch.flatten(x, 1)
        # H,W = self.layers[0].input_resolution[0],self.layers[0].input_resolution[1]
        # print("H, W" , H, W)
        outs = []
